// Implementation for GraphFFSM class
#include "graphFFSM.h"
#include <iostream>
#include <cstdlib>
#include <tuple>
 ...
 */
bool GraphFFSM::addEdge(int idx1, int idx2, int edge_weight) {
  if (idx1 >= node_count) {
...
/* displayGraph function prints the GraphFFSM on screen with same format as the input
 */
void GraphFFSM::displayGraph() {
  cout << "# GraphFFSM name\n";
  cout << name << endl;
...
// GetName function
string GraphFFSM::getName() {
  return name;
}

// getNode function
GraphNode GraphFFSM::getNode(int n) {
  assert(n < node_count);
  return nodes[n];
}
...
string GraphFFSM::getEdgeLabel(int row, int col) {
