...
bool AdjMatrix::extension(vector<GraphFFSM*>& graph_db, vector<AdjMatrix>& extension_list) {
...
int AdjMatrix::getUniqFrequency() const {
  ...
  for (int i = 0; i < graph_count; i++) {
      if (!emb_list[i].empty()) {
          support++; // pattern appears at least once in graph i
      }
  }
  return support;
}
...
Graph AdjMatrix::toGraph() const {
    Graph g;

    // Extract vertex labels from the diagonal
    for (int i = 0; i < m.size(); ++i) {
        g.setVertexName(i, stoi(m[i][i]));
    }

    // Extract edges from the off-diagonal
    for (int i = 0; i < m.size(); ++i) {
        for (int j = i + 1; j < m.size(); ++j) {
            if (stoi(m[i][j]) != 0) {
                int label = stoi(m[i][j]);
                g.addEdge(i, j, m[i][j]); // Use i and j as vertex IDs
            }
        }
    }
